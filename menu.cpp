...
void drawInGameStats()
{
	if(oldHUD.coins != HUD.coins||oldHUD.score != HUD.score||oldHUD.lifes != HUD.lifes)
	{
		...
		if(HUDTEXT)
			delete[] (HUDTEXT);
		HUDTEXT = new char[70];
		//sprintf(HUDTEXT,"总分 : %d    金币 : %d    命数 : %d",HUD.score,HUD.coins,HUD.lifes);
		sprintf(HUDTEXT,"总分 : %d    金币 : %d    命数 : %d",HUD.score,HUD.coins,HUD.lifes);
		hudSurf = TTF_RenderText_Blended(font,HUDTEXT,HUDColorbg);	
		//hudSurf = TTF_RenderUNICODE_Blended(font, '0x4F60',HUDColorbg);	
		tmp_rect.x=1;
		tmp_rect.y=1;
	    hudoverlay = TTF_RenderText_Blended(font,HUDTEXT,HUDColor);
	    //hudoverlay = TTF_RenderUNICODE_Blended(font, '0x4F60',HUDColor);
